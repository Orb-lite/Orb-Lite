  ...
  return (
    <div className="rounded-md border border-border/60 p-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-end gap-2">
          ...
          {url ? (
            ...
            >
              {request.isPending ? <Loader2 className="size-4 animate-spin" /> : <Play className="size-4" />}
              Empezar a grabar
            </button>
          )}
        </div>
      ...
      <div className="mt-3 aspect-video overflow-hidden rounded-md bg-black/80">
        {url ? (
          <video ref={videoRef} controls playsInline muted className="size-full object-contain" />
        ) : (
      ...
      {request.isError ? (
        <p className="mt-2 text-sm text-destructive">
          {request.error instanceof Error ? request.error.message : 'No se pudo iniciar la transmisión.'}
        </p>
      ) : null}
      ...
      <p className="mt-3 flex items-start gap-2 text-xs text-muted-foreground">
        ...
        <span>
          En {resolution} el video consume aproximadamente <strong>{consumption.label}</strong> de datos por hora de
          transmisión. Cada solicitud de grabación genera consumo en el plan de datos de la unidad; a mayor resolución,
          mayor costo.
        </span>
      </p>
    </div>
