...
export function WialonUnitDetail({
  ...
  async function runCommand(name: string, link: string) {
    ...
    try {
      await commandFn({
        data: { host: session.host, sid: session.sid, unitId, commandName: name, linkType: link },
      })
          ...
          <div className="mt-5 space-y-6 text-sm">
            ...
            <section>
              ...
              {data.sensors.length === 0 ? (
                <p className="mt-2 text-muted-foreground">Esta unidad no tiene sensores configurados.</p>
              ) : (
                <ul className="mt-2 space-y-1">
                  {data.sensors.map((sensor) => (
                    <li key={sensor.id} className="flex justify-between gap-3 border-b border-border/40 py-1">
                ...
                <div className="mt-2 grid gap-1 sm:grid-cols-2">
                  {data.params.map((param) => (
                    <p key={param.key} className="text-muted-foreground">
                      <span className="font-semibold text-foreground">{param.key}:</span> {param.value}
                    </p>
                  ))}
                </div>
              </section>
