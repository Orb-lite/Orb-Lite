...
export const Route = createFileRoute("/wialon/video")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Consulta las cámaras configuradas de tus unidades ORB-FULL.",
      },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: () => (
    <WialonGuard>{(session) => <VideoView session={session} />}</WialonGuard>
  ),
});
    ...
    return (
      <div className="rounded-lg border border-border/60 bg-card p-8 text-center shadow-sm">
        ...
        <h2 className="mt-3 font-display text-lg font-bold uppercase tracking-wide">
          Módulo exclusivo para ORB-FULL
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          La visualización de cámaras y video en vivo está disponible únicamente
          para cuentas ORB-FULL (Wialon Full).
        </p>
      </div>
    );
  }
...
function FullVideoView({ session }: { session: WialonSession }) {
  ...
  const videoUnits = useQuery({
    ...
    queryFn: () =>
      videoUnitsFn({ data: { host: session.host, sid: session.sid } }),
  });
  const selectedId = unitId ?? videoUnits.data?.units[0]?.id ?? null;
  const selectedUnit = videoUnits.data?.units.find(
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4 rounded-lg border border-border/60 p-5">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Consulta las cámaras registradas y su estado. El visor oficial de
            Wialon reproduce el video en vivo y las grabaciones de forma segura.
          </p>
        </div>
        <a
        ...
        >
          {videoUnits.data?.units.map((unit) => (
            <option key={unit.id} value={unit.id}>
              {unit.name} · {unit.cameraCount} cámara
              {unit.cameraCount === 1 ? "" : "s"}
            </option>
          ))}
        </select>
      </label>

      {videoUnits.isLoading ? (
        <p className="text-sm text-muted-foreground">
          Buscando unidades con video…
        </p>
      ) : null}
      {videoUnits.isError ? (
      ...
      {videoUnits.data && videoUnits.data.units.length === 0 ? (
        <div className="rounded-lg border border-border/60 p-5 text-sm text-muted-foreground">
          No hay unidades con cámaras configuradas o tu token no tiene el
          permiso “Ver propiedades detalladas del elemento”.
        </div>
      ) : null}

      {video.isLoading ? (
        <p className="text-sm text-muted-foreground">Consultando cámaras…</p>
      ) : null}
      {video.isError ? (
        <div className="rounded-lg border border-destructive/40 p-5 text-sm text-destructive">
          ...
          <p className="mt-2 text-muted-foreground">
            Tu usuario necesita permisos para ver propiedades detalladas de la
            unidad y el servicio de video debe estar habilitado.
          </p>
        </div>
      ) : null}
      ...
      {video.data ? (
        <section className="rounded-lg border border-border/60 p-5">
          <h3 className="font-display text-base font-bold uppercase tracking-wide">
            {selectedUnit?.name ?? "Unidad"} · {video.data.cameras.length}{" "}
            cámara(s)
          </h3>
          {video.data.cameras.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              Esta unidad no tiene cámaras configuradas o no tienes permiso para
              consultarlas.
            </p>
          ) : (
